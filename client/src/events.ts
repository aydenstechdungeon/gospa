...
export function parseEventString(eventStr: string): {
  event: string;
  modifiers: EventModifier[];
} {
...
export function on<K extends keyof HTMLElementEventMap>(
  ...
  eventStr: string,
  handler: EventHandler<
    HTMLElementEventMap[K] extends Event ? HTMLElementEventMap[K] : Event
  >,
): () => void {
      ...
      (event) => {
        ...
        while (target && target !== root) {
          ...
          if (attr) {
            ...
            if (
              eventStr === eventName ||
              (eventStr === "focus" && eventName === "focusin") ||
              (eventStr === "blur" && eventName === "focusout")
            ) {
              ...
              if (islandEl) {
                ...
                if (
                  ...
                  islandInstance.handlers[handlerName]
                ) {
