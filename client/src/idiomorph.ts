...
export interface IdiomorphConfig {
  ...
  head?: {
    ...
    afterHeadMorphed?: (
      el: Element,
      info: { added: Node[]; kept: Element[]; removed: Element[] },
    ) => void;
  };
}
...
export const Idiomorph = (function () {
  ...
  function createMorphContext(
    ...
    const oldRoot =
      oldNode instanceof SlicedParentNode ? oldNode.originalNode : oldNode;
    const newRoot =
      newNode instanceof SlicedParentNode ? newNode.originalNode : newNode;
  ...
  function populateIdMap(
    ...
    if (node instanceof Element || node instanceof DocumentFragment) {
      ...
      const childrenWithIds =
        (node instanceof Element ? node : (node as any)).querySelectorAll?.(
          "[id]",
        ) || [];
  ...
  function morphAttributes(oldNode: Element, newNode: Element, ctx: any) {
    for (const attr of Array.from(newNode.attributes)) {
      if (
        ctx.callbacks.beforeAttributeUpdated(attr.name, oldNode, "update") !==
        false
      ) {
    ...
    for (const attr of Array.from(oldNode.attributes)) {
      if (!newNode.hasAttribute(attr.name)) {
        if (
          ctx.callbacks.beforeAttributeUpdated(attr.name, oldNode, "remove") !==
          false
        ) {
