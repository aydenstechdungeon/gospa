...
export class StreamingManager {
  ...
  private setupStreamHandler(): void {
    ...
    const existingHandler = (globalThis as unknown as Record<string, unknown>)
      .__GOSPA_STREAM__;

    (globalThis as unknown as Record<string, unknown>).__GOSPA_STREAM__ = (
      chunk: StreamChunk,
    ) => {
  ...
  private handleErrorChunk(chunk: StreamChunk): void {
    ...
    document.dispatchEvent(
      new CustomEvent("gospa:stream-error", {
  ...
  private async hydrateIsland(island: IslandData): Promise<void> {
    ...
    const islandManager = (globalThis as unknown as Record<string, unknown>)
      .__GOSPA_ISLAND_MANAGER__;
    if (
      islandManager &&
      typeof (
        islandManager as {
          hydrate: (id: string, data: IslandData) => Promise<void>;
        }
      ).hydrate === "function"
    ) {
      await (
      ...
      ).hydrate(island.id, island);
    }

    // Dispatch hydration event
    element.dispatchEvent(
      new CustomEvent("gospa:hydrated", {
        detail: { island },
      }),
    );
