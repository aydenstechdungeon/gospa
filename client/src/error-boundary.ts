 ...
 */
const globalErrorHandlers = new Set<(error: Error, componentId: string) => void>();

/**
 * Register a global error handler
 */
export function onComponentError(handler: (error: Error, componentId: string) => void): () => void {
...
export function withErrorBoundary(
	...
	wrapAction: <T>(actionFn: (...args: unknown[]) => T) => (...args: unknown[]) => T;
	clearError: () => void;
	...
	const handleError = (error: Error): void => {
		...
		if (element) {
			const fallbackEl = typeof config.fallback === 'function'
	...
	const wrapMount = (mountFn: () => void | (() => void)): () => void | (() => void) => {
		return () => {
			const state = getState();
			if (state.hasError) {
	...
	const wrapAction = <T>(actionFn: (...args: unknown[]) => T): (...args: unknown[]) => T => {
		return (...args: unknown[]): T => {
			const state = getState();
			if (state.hasError) {
				throw new Error(`Component ${componentId} is in error state: ${state.error?.message}`);
			}
 ...
 */
export function getErrorBoundaryState(componentId: string): ErrorBoundaryState | undefined {
