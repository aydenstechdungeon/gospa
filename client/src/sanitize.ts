...
export const PURIFY_CONFIG: DOMPurifyConfig = {
	...
	ALLOWED_TAGS: [
		...
		'em', 'strong', 'i', 'b', 'u', 'small', 'mark', 'del', 'ins',
		...
		'a',
		// Other semantic
		'address', 'time', 'data', 'meter', 'progress', 'output',
		// SVG (safe subset - no animation events)
		'svg', 'path', 'rect', 'circle', 'line', 'polyline', 'polygon', 'ellipse', 'g',
		'defs', 'use', 'symbol', 'text', 'tspan',
	...
	ALLOWED_ATTR: [
		...
		// Audio/Video
		'controls', 'loop', 'muted', 'preload', 'autoplay', 'poster',
		// Tables
	...
	FORBID_TAGS: [
		...
		'meta', 'link', 'base', 'head', 'body', 'html',
		'noscript', 'noframes', 'noembed',
		// Dangerous SVG/Math
		'math', 'animate', 'animateTransform', 'animateMotion', 'set'
	],
	...
	FORBID_ATTR: [
		...
		'onchange', 'onsubmit', 'oninput', 'onkeydown', 'onkeypress', 'onkeyup',
		'onmousedown', 'onmouseup', 'onmousemove', 'onmouseenter', 'onmouseleave',
		'ondblclick', 'oncontextmenu', 'onwheel', 'onscroll', 'onresize',
		'onselect', 'onselectionchange', 'oncut', 'oncopy', 'onpaste',
		...
		// Form/DOM Clobbering
		'name', 'form', 'formaction', 'formmethod', 'formtarget', 'formenctype', 'formnovalidate',
		// Dangerous URLs
		'xlink:href', 'xmlns:xlink',
		// Other
		'action', 'manifest', 'http-equiv', 'content'
	],
...
export async function sanitize(html: string): Promise<string> {
	try {
		...
		return typeof result === 'string' ? result : (result as unknown as TrustedHTML).toString();
	} catch (error) {
