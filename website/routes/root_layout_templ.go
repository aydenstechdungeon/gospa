...
func setupGlobalActions() templ.ComponentScript {
	return templ.ComponentScript{
		Name: `__templ_setupGlobalActions_10ac`,
		Function: `function __templ_setupGlobalActions_10ac(){// Global switchLang function for DualCodeBlock component
	...
	window.switchLang = function(btn, lang) {
		...
		// Update global toggles in header
		const globalActive = ['bg-[var(--accent-primary)]', 'text-black', 'shadow-sm'];
		const globalInactive = ['text-[var(--text-muted)]', 'hover:text-[var(--text-primary)]'];
		
		document.querySelectorAll('[data-global-lang]').forEach(b => {
			if (b.getAttribute('data-global-lang') === targetLang) {
				globalActive.forEach(c => b.classList.add(c));
				globalInactive.forEach(c => b.classList.remove(c));
			} else {
				globalInactive.forEach(c => b.classList.add(c));
				globalActive.forEach(c => b.classList.remove(c));
			}
		});
	};
	...
	document.addEventListener('click', (e) => {
		...
		Call:       templ.SafeScript(`__templ_setupGlobalActions_10ac`),
		CallInline: templ.SafeScriptInline(`__templ_setupGlobalActions_10ac`),
	}
}
