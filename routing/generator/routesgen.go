...
func (g *RouteTypeScriptGenerator) generateRouteDataContracts(sb *strings.Builder) {
	...
	sb.WriteString("function headersToRecord(headers?: HeadersInit): Record<string, string> {\n")
	sb.WriteString("  if (!headers) return {};\n")
	sb.WriteString("  if (headers instanceof Headers) {\n")
	sb.WriteString("    const record: Record<string, string> = {};\n")
	sb.WriteString("    headers.forEach((value, key) => {\n")
	sb.WriteString("      record[key] = value;\n")
	sb.WriteString("    });\n")
	sb.WriteString("    return record;\n")
	sb.WriteString("  }\n")
	sb.WriteString("  if (Array.isArray(headers)) return Object.fromEntries(headers);\n")
	...
	sb.WriteString("  const actionBody = body ?? requestInit.body ?? null;\n")
	sb.WriteString("  if (csrfToken && actionBody instanceof FormData && !actionBody.has('_csrf')) {\n")
	sb.WriteString("    actionBody.set('_csrf', csrfToken);\n")
	sb.WriteString("  }\n")
	sb.WriteString("  const headers: Record<string, string> = {\n")
	sb.WriteString("    Accept: 'application/json',\n")
	sb.WriteString("    'X-Gospa-Enhance': '1',\n")
	sb.WriteString("    ...headersToRecord(requestInit.headers),\n")
	sb.WriteString("  };\n")
	sb.WriteString("  if (csrfToken && !(actionBody instanceof FormData)) {\n")
	sb.WriteString("    headers['X-CSRF-Token'] = csrfToken;\n")
	sb.WriteString("  }\n")
	...
	sb.WriteString("    headers,\n")
	sb.WriteString("    body: actionBody,\n")
	sb.WriteString("  });\n")
